...
nrow(Shapes)
CDs <- select( Shapes, "SESSN","ST","DISTRICT","PolsbyW","STpolsby","RSTPolsby","CSTavg")

#### Calc Relative district compactness percentile adjusted for state geography
Model <- read.csv( file= "ModelParams_ALL.csv")
#Model <- read.csv( file= "ModelParams_X.csv")    ### Use model based on removed outliers

Intercept <- Model$Parameters[1]
RSTPcoeff <- Model$Parameters[2]
CSTAcoeff <- Model$Parameters[3]
Phi <- Model$Parameters[4]

CDs<- CDs%>% mutate( Mu = Intercept + RSTPcoeff*RSTPolsby + CSTAcoeff*CSTavg )
CDs <- CDs%>% mutate( Dscore = pbetar( PolsbyW,Mu,Phi))  
